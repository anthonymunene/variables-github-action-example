...
export function getDependencyFiles(): string[] {
    return getFilesFromDirectory(TOKENS_DIR).filter((file) => 
        file.endsWith('.json') && 
        (file.includes(VARIABLE_COLLECTIONS.core) || 
         file.includes(VARIABLE_COLLECTIONS.layout) ||
         file.includes('screensize'))
    );
}
