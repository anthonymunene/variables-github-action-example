...
export const processContentTokens = (dictionary: any, brand: string, buildPath: string) => {
    ...
    const contentTokens = dictionary.allTokens.filter((token: any) => token.path[0] === 'content');
    ...
    fs.writeFileSync(path.join(contentDir, 'content.json'), JSON.stringify(contentOutput, null, 2));
    // eslint-disable-next-line no-console
...
export const isNumber = (token: TransformedToken) => {
    return token.$type === 'number' && typeof token.original.$value === 'number';
};
